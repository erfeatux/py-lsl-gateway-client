...
if not __sslcontext:
	if not os.path.isfile('LindenLab.crt'):
		urllib.request.urlretrieve(
				'https://raw.githubusercontent.com/secondlife/llca/master/LindenLab.crt', __llcacrt)
...
@validate_call
def __exceptionByResp(resp = Annotated[aiohttp.ClientResponse, Field()]) -> excepts.HTTPException:
	code = resp.status
	if resp.reason:
		reason = resp.reason
	else:
	...
	match code:
		...
		case 203:
			err = excepts.HTTPNonAuthoritativeInformation(reason=reason, headers=resp.headers)
		case 204:
		...
		case 300:
			err = excepts.HTTPMultipleChoices(reason=reason, headers=resp.headers,
												location=resp.headers['Location'])
		case 301:
			err = excepts.HTTPMovedPermanently(reason=reason, headers=resp.headers,
												location=resp.headers['Location'])
		case 302:
			err = excepts.HTTPFound(reason=reason, headers=resp.headers, location=resp.headers['Location'])
		case 303:
			err = excepts.HTTPSeeOther(reason=reason, headers=resp.headers, location=resp.headers['Location'])
		case 304:
			err = excepts.HTTPNotModified(reason=reason, headers=resp.headers)
		case 305:
			err = excepts.HTTPUseProxy(reason=reason, headers=resp.headers, location=resp.headers['Location'])
		case 307:
			err = excepts.HTTPTemporaryRedirect(reason=reason, headers=resp.headers,
												location=resp.headers['Location'])
		case 308:
			err = excepts.HTTPPermanentRedirect(reason=reason, headers=resp.headers,
												location=resp.headers['Location'])
		...
		case 405:
			err = excepts.HTTPMethodNotAllowed(reason=reason, headers=resp.headers,
								method=resp.method, allowed_methods=resp.headers['Allow'].split())
		case 406:
			err = excepts.HTTPNotAcceptable(reason=reason, headers=resp.headers)
		case 407:
			err = excepts.HTTPProxyAuthenticationRequired(reason=reason, headers=resp.headers)
		case 408:
		...
		case 413:
			err = excepts.HTTPRequestEntityTooLarge(reason=reason, headers=resp.headers, max_size=0,
		...
		case 416:
			err = excepts.HTTPRequestRangeNotSatisfiable(reason=reason, headers=resp.headers)
		case 417:
		...
		case 431:
			err = excepts.HTTPRequestHeaderFieldsTooLarge(reason=reason, headers=resp.headers)
		case 451:
			err = excepts.HTTPUnavailableForLegalReasons(reason=reason, headers=resp.headers, link=None)
		...
		case 511:
			err = excepts.HTTPNetworkAuthenticationRequired(reason=reason, headers=resp.headers)
